...
$app->group($root, function() use ($trueRoot, $settings, $access, $container) {
	...
	$this->group('/api/v1', function() use ($settings, $access, $container) {
		...
		$this->post('/sync/{id:\d+}/syncfile', \App\Controllers\SyncController::class . ':syncFile')
			->setName('api.sync.file');
	...
	$this->get('/projects/{id:\d+}', \App\Controllers\ProjectController::class . ':item')->setName('main.project');
